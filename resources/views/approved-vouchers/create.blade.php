            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <?php $authorized = \DB::table('users')->leftJoin('roles', 'users.role_id', '=', 'roles.id')
                        ...
                        ->where('permissions.key', 'add_approved_vouchers')->exists(); ?>
                    @if ($authorized)
                        @if (session('status'))
                        ...
                        <form method="POST" action="/approved-vouchers">
                            ...
                            <div class="form-group custom-control-inline">
                                ...
                                <input type="date" class="form-control @error('approved_at') is-danger @enderror" id="approved_at" name="approved_at" value="{!! old('approved_at') !!}">
                            </div>
                            <div class="form-group custom-control-inline">
                    ...
                    @else
                        You are not authorized to add approved vouchers.
                    @endif
                </div>
            </div>
        </div>
