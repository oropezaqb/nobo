            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <?php $authorized = \DB::table('users')->leftJoin('roles', 'users.role_id', '=', 'roles.id')
                        ...
                        ->where('permissions.key', 'read_approved_vouchers')->exists(); ?>
                    @if ($authorized)
                        @if (session('status'))
                        ...
                        <form method="POST" action="/approved-vouchers">
                            ...
                            <div class="form-group custom-control-inline">
                                ...
                                <input type="number" id="voucher_number" name="voucher_number" style="text-align: right;" data-id="" class="form-control"
                                    value="{!! old('voucher_number', $approvedVoucher->voucher->number) !!}" oninput="getVoucher()" disabled>
                            </div>
                            <div class="form-group">
                                <label for="payee_id">Payee:&nbsp;</label>&nbsp;
                                <input list="payee_ids" id="payee_id0" onchange="setValue(this)" data-id="" class="custom-select"
                                    value="{!! old('payee_id0', $approvedVoucher->voucher->bill->payee->name) !!}" disabled>
                            </div>
                            <br>
                            <div class="form-group custom-control-inline">
                                <label for="bill_number">Bill no. </label>
                                <input class="form-control" type="text" name="bill_number" id="bill_number"
                                    value="{{ old('bill_number', $approvedVoucher->voucher->bill->bill_number) }}" disabled>
                            </div>
                            <div class="form-group custom-control-inline">
                                ...
                                <input type="date" class="form-control @error('period_start') is-danger @enderror" id="period_start" name="period_start"
                                    value="{!! old('period_start', $approvedVoucher->voucher->bill->period_start) !!}" disabled>
                            </div>
                            <div class="form-group custom-control-inline">
                                ...
                                <input type="date" class="form-control @error('period_end') is-danger @enderror" id="period_end" name="period_end"
                                    value="{!! old('period_end', $approvedVoucher->voucher->bill->period_end) !!}" disabled>
                            </div>
                            <div class="form-group">
                                <label for="particulars">Particulars </label>
                                <textarea class="form-control" rows="5" id="particulars" name="particulars" disabled>{{ old('particulars', $approvedVoucher->voucher->bill->particulars) }}</textarea>
                            </div>
                            <br>
                            <div class="form-group custom-control-inline">
                                <label for="amount">Amount</label>&nbsp;
                                <input type="number" class="form-control amount" id="amount" name="amount" step="0.01" style="text-align: right;"
                                    value="{!! old('amount', $approvedVoucher->voucher->bill->amount) !!}" disabled>
                            </div>
                            <br><br>
                            <div class="form-group custom-control-inline">
                                <label for="date">Document date:&nbsp;</label>&nbsp;
                                <input type="date" class="form-control @error('date') is-danger @enderror" id="date" name="date" value="{!! old('date', $approvedVoucher->voucher->date) !!}" disabled>
                            </div>
                            <div class="form-group custom-control-inline">
                                <label for="posted_at">Posting date:&nbsp;</label>&nbsp;
                                <input type="date" class="form-control @error('posted_at') is-danger @enderror" id="posted_at" name="posted_at" value="{!! old('posted_at', $approvedVoucher->voucher->posted_at) !!}" disabled>
                            </div>
                            <div class="form-group custom-control-inline">
                                ...
                                <input type="number" class="form-control amount" id="payable_amount" name="payable_amount" step="0.01" style="text-align: right;"
                                    value="{!! old('payable_amount', $approvedVoucher->voucher->payable_amount) !!}" disabled>
                            </div>
                            <div class="form-group">
                                <label for="remarks">Remarks </label>
                                <textarea class="form-control" rows="5" id="remarks" name="remarks" disabled>{{ old('remarks', $approvedVoucher->remarks) }}</textarea>
                            </div>
                            <br>
                            <div class="form-group custom-control-inline">
                                <label for="approved_at">Date approved:&nbsp;</label>&nbsp;
                                <input type="date" class="form-control @error('approved_at') is-danger @enderror" id="approved_at" name="approved_at" value="{!! old('approved_at', $approvedVoucher->approved_at) !!}" disabled>
                            </div>
                            <div class="form-group custom-control-inline">
                                <label for="endorsed_at">Date endorsed:&nbsp;</label>&nbsp;
                                <input type="date" class="form-control @error('endorsed_at') is-danger @enderror" id="endorsed_at" name="endorsed_at" value="{!! old('endorsed_at', $approvedVoucher->endorsed_at) !!}" disabled>
                            </div>
                            <div class="form-group custom-control-inline">
                        ...
                        <div style="clear: both;">
                            <div style="display: inline-block;">
                                <button class="btn btn-outline-primary" onclick="location.href = '/approved-vouchers/{{ $approvedVoucher->id }}/edit';">Edit</button>
                            </div>
                            <div style="display: inline-block;">
                                <form method="POST" action="/approved-vouchers/{{ $approvedVoucher->id }}">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @else
                        You are not authorized to view approved voucher details.
                    @endif
                </div>
            </div>
        </div>
