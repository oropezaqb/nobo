            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    @if ($browse)
                        ...
                        <form method="GET" action="/cancelled-vouchers">
                            @csrf
                        ...
                        @forelse ($cancelledVouchers as $voucher)
                            <div id="content">
                                <div id="title">
                                    <div style="display:inline-block;"><button class="btn btn-link" onclick="location.href = '{{ \App\Models\CancelledVoucher::find($voucher->id)->path(); }}'">View</button></div>
                                    <div style="display:inline-block;"><button class="btn btn-link" onclick="location.href = '/cancelled-vouchers/{{ $voucher->id }}/edit';">Edit</button></div>
                                    <div style="display:inline-block;"><form method="POST" action="/cancelled-vouchers/{{ $voucher->id }}">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <p>No cancelled vouchers recorded yet.</p>
                        @endforelse
                        {{ $cancelledVouchers->links() }}
                    @else
                        You are not authorized to browse cancelled vouchers.
                    @endif
                </div>
            </div>
        </div>
